  ...
  def login_fiber()
    fiber = Fiber.new do
      loop do
        ...
        userDomain:tell_domain(Time.now),
...
#ready for pinger to send the answer to server
  ...
  def cause_relog_thread
    Thread.new {
      pp "#{Thread.current} started 4*3600"
      sleep(4*3600);
      20.times {print"\u{2588 2588 2588 20}"}
      print "#{Thread.current} killing values \u{0a}";
      kill_values();
    }
  end
...
#reports if presence_check_time is listed in ANY "presenceCheckList"
  def check_presence_updated
    ...
    @list_json['presenceCheckList'].each{|📦| updated = true if #1f4e6
      (📦['startTime']..📦['endtime']).include?(@presence_check_time.to_i+4) }
    return updated;
  end
...
#sort "startdate: enddate:" to implement
    ...
    if (diff > (7*60)) #time diff more than 7 min,
      kill_values(); #will cause login call
    end
  end#process_and_create_overview

  def kill_values
    @ping_token,@answer,@question,@presence_check_time = nil;
